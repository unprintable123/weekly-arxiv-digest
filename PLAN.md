...
状态：已完成基础实现（package.json、tsconfig、CLI help、配置 schema 与依赖声明）。运行验证待 Node 20+/pnpm 环境。
...
状态：已完成（ISO 周窗口、领域类型、稳定 hash/序列化、interest 分类解析与自然语言类别映射）。
...
状态：已完成首版实现（可通过 `source.provider` 选择 `papers.cool` HTML 或 `arxiv` Export API；两者均支持分页、日期过滤、限速/超时/重试、缓存和版本去重；每个 provider 只访问自身数据源；不请求 PDF）。fixture 契约测试待补。
...
状态：已完成核心 schema 与缓存 API（papers、fetch/relevance/translation、runs、run_papers、llm_errors、stats/prune）。
...
状态：已完成首版适配器与 JSON 校验、评分/翻译重试及独立缓存；需在实际 pi 包版本上补 API 适配测试。
...
状态：已完成 Markdown renderer、排序、转义、外链白名单、原子写入与 preview 数据通路。
...
状态：已完成 run、preview、cache stats/prune 命令、运行记录和 README；retry、dry-run、并发参数、pnpm-lock 生成与真实源验收待在 Node 20+/pnpm 环境补齐。
